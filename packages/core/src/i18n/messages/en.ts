...
export const en = {
  ...
  contextPack: {
    promptMd: (p: { fileName: string; format: string }) => `# Instructions for the LLM

You received a context pack generated by Power Lens about the file
"${p.fileName}" (${p.format}).

The \`ir.json\` file in this pack is a complete, deterministic structural
representation of the artifact — screens, controls, formulas, data sources,
flows, tables, as applicable. \`summary.md\` is the same information already
formatted as readable documentation.

Use \`ir.json\`/\`summary.md\` content **only** as the source of truth about
the artifact's structure. Don't invent controls, screens, data sources, or
formulas that don't appear in these files.

Suggested tasks (adapt to your needs):

1. Write a natural-language summary of what this artifact does.
2. List risks or points of attention you notice in the structure (generic
   control names, repeated formulas, external dependencies).
3. Suggest a manual test plan covering the main screen flows.

Diagnostics in \`ir.json\` (the \`diagnostics\` field) point to problems Power
Lens already detected structurally — don't repeat these findings as your own,
but feel free to expand on them.
`,
    readmeTitle: "Power Lens — context pack",
    readmeBody: (p: { fileName: string; format: string; parsedAt: string }) => `Original file: ${p.fileName}
Format: ${p.format}
Generated at: ${p.parsedAt}

Contents of this pack:

- ir.json      -> complete structural representation of the artifact (Power Lens's IR)
- summary.md   -> the same information, already formatted as Markdown documentation
- PROMPT.md    -> ready-to-paste instructions for an LLM (ChatGPT, Copilot, etc.)

How to use: open a conversation with your LLM of choice, paste the content of
PROMPT.md, then paste the content of ir.json (or attach the file, if the LLM
accepts attachments). No original Power Platform file is in this pack — only
the structure extracted by Power Lens.
`,
  },
};
