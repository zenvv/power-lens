 ...
 * CanvasApps/*.msapp, reused via the msapp parser) and leaves everything
 * else (Workflows/, customizations.xml tables) as an honest "not parsed in
 * this phase yet" diagnostic rather than guessing at their shape. Never
 * throws — every failure degrades to a Diagnostic on the returned document.
 */
export function parseSolution(bytes: Uint8Array, source: SolutionSource): PowerLensDocument {
  ...
  const workflowPaths = Object.keys(entries).filter((path) => /^Workflows\/.*\.json$/i.test(path));
  if (workflowPaths.length > 0) {
    diagnostics.push({
      code: "PL305",
      severity: "info",
      message: `${workflowPaths.length} arquivo(s) em Workflows/ encontrados, mas fluxos dentro de solutions ainda não são parseados nesta fase: ${workflowPaths.join(", ")}`,
    });
  }
